...
def main(
    ...
    headers: Annotated[
        Optional[List[str]],
        typer.Option("--header", "-H", help="Custom headers for SSE/Streamable HTTP servers (format: 'Key: Value')")
    ] = None,
):
    server_command = None
    if not config_path:
    ...
    if headers:
        for header in headers:
            try:
                ...
                typer.echo(f"Warning: Invalid header format '{header}'. Expected 'Key: Value'")
                continue
