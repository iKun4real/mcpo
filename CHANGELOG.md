...
- 🔐 **Custom Headers Support for SSE/Streamable HTTP**: Added support for custom request headers when connecting to SSE and Streamable HTTP MCP servers. This enables authentication and other custom header requirements.
  - CLI support: Use `--header "Key: Value"` or `-H "Key: Value"` to add custom headers
  - Config file support: Add `"headers": {"Key": "Value"}` to server configurations
  - Multiple headers supported for both CLI and config file usage
  - Proper error handling for invalid header formats
